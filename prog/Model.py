...
#Enregistrer le chemin menant aux fichiers static 
...
class Session:
    ...
    def tableau_by_search(self):
        ...
        et elle gère les erreurs, si il y'a rien comme filtrage, ou si la requête échoue (pas de données). Cette dataframe va être utilisée 
        pour afficher toutes les informations nécessaire + les graphiques dans notre site web, la mise en forme de ces données se font dans d'autres
        méthodes.
        """
        ...
        try :
            ...
            #Notifier à l'utilisateur quand il choisi des infos tout simplement pas dispo sur l'API du tout (ANC>2017)
                ...
                requete = f"""
                    ...
                    OR type_collectivite LIKE ?) limit 3000;
                """
                ...
                if codes_communes:
                    ...
                    """
                        ...
                        where nom_service LIKE ? AND Descriptif.code_commune in ({truc}) limit 3000;"""
    ...
    def jauge(self,tableau):
        """
        Est utilisé lorsque l'utilisateur a filtré avec un mot clé correspondant à un indicateur (pour plus de précisions sur les indicateurs,
        veuillez aller sur la page documentation). Elle va évaluer la quantité moyenne de l'indicateur en France en récupérant les données de
        la colonne "valeur" de la dataframe. Elle retourne le pourcentage de l'indicateur dans la zone française entière en fonction du
        min de l'indicateur enregistré et du max enregistré dans la dtframe.  
        """
    ...
    def constr_graphe(self,tableau):
        """Ne construit pas un graphique, mais regroupe toutes les données nécessaire pour construire les graphiques dans un fichier 
        json du nom de graph_data_region.json. Elle enregistre les valeurs moyenne des indicateurs, l'évolution de x indicateur par année
        et enregistre les couleurs à afficher sur la carte si la jauge est affiché (si l'user recherche un indicateur spécifique dans la barre de recherche).
        Les graphiques sont construites avec chart.js dans un fichier js dans le dossier static."""
        con = duck.connect()
        ...
        con.close()
        filepath = os.path.join(path2, "graph_data_region.json")
    ...
    def processus_tab_graphe(self):
        """fonction qui enchaîne les éxecutions pour le processus d'affichage des données. Retourne un tableau html et le pourcentage de x 
        indicateur en fonction du max et du min dans la jauge."""
        test_indicateur=""
        try:
