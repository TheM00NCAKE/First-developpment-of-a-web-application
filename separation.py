...
def processus(min,max,valeur):
    ...
    with ThreadPoolExecutor() as executeur:
        ...
        img=Image.open(f'ImgTemp{valeur.upper()}.png')
    ...
    Image.fromarray(canvas).save(f"ImgTest{valeur.upper()}.png",compress_level=0)
...
"""IDEE : QUAND LE SITE CHARGE, DFS EST EXEC UNE FOIS ET ELLE EST STOCKE EN CACHE, POUR LA VISUALISATION ON POURRA JUSTE EXEC DFD ET LE TOUR EST JOUE
fdd : read_dataframe : 11.269212245941162 - 13.964303731918335
      open_dataset quand on fait autre chose en attendant : 7.9101502895355225 - 8.26500153541565
